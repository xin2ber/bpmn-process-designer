  ...
  <div class="panel-tab__content">
    <el-form size="mini" label-width="90px" @submit.native.prevent>
      <el-form-item label="类型">
        <el-select  v-model="formData.type" class="select" placeholder="请选择" filterable allow-create @change="changeType">
...
export default {
  ...
  methods: {
    changeType(val) {
      ...
      this.updateProperties( {val:this.formData.value});
      console.log(this.formData)
      let index;
      for (index in this.implementationTypes) {
        console.log(index);
        if(index !== val) {
          this.updateProperties( {index:null} );
        }
      }
    }
  }
