<template>
  <div class="panel-tab__content">
    <el-table :data="formData.listener" size="mini" max-height="240" border fit>
    ...
    <!-- 监听器 编辑/创建 部分 -->
      <listener-param v-if="showParamDialog" :value="formData.listener[nowIndex].params" @close="finishConfigParam" />
...
export default {
  ...
  mounted() {
    this.formData.listener = this.element.businessObject.extensionElements?.values
  ...
  methods: {
    add() {
      this.formData.listener.splice(this.formData.listener.length, 0, {});
    },
    remove(index) {
      this.formData.listener.splice(index, 1);
    },
    configParam(index) {
      this.nowIndex = index
      const nowObj = this.formData.listener[index]
      if (!nowObj.params) {
        nowObj.params = []
      }
    ...
    finishConfigParam(param) {
      ...
      // hack 数量不更新问题
      const cache = this.formData.listener[this.nowIndex]
      cache.params = param
      this.$set(this.formData.listener[this.nowIndex], this.nowIndex, cache)
      this.nowIndex = null
    },
    updateElement() {
      console.log(this.formData);
      if (this.formData.listener?.length) {
        ...
        this.formData.listener.forEach(item => {
